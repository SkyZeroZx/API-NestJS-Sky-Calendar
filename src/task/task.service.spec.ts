...
describe('TaskService', () => {
  ...
  it('Validamos findByTask', async () => {
    ...
    expect(spyAddSelect).toHaveBeenNthCalledWith(1, 'USER.nombre', 'nombre');
    expect(spyAddSelect).toHaveBeenNthCalledWith(2, 'USER.apellidoPaterno', 'apellidoPaterno');
    expect(spyAddSelect).toHaveBeenNthCalledWith(3, 'USER.apellidoMaterno', 'apellidoMaterno');
    expect(spyInnerJoin).toHaveBeenNthCalledWith(
      1,
      TaskToUser,
      'TASK_TO_USER',
