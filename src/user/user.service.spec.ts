...
describe('UserService', () => {
  ...
  it('Validamos Update Ok', async () => {
    // Hacemos que retorne un usuario mockeado con estado creado
    const spyUpdateRepository = jest
      .spyOn(mockService, 'update')
    ...
    expect(spyCreate).toBeCalledWith({
      nombre: UserServiceMock.updateUser.nombre,
      apellidoMaterno: UserServiceMock.updateUser.apellidoMaterno,
      apellidoPaterno: UserServiceMock.updateUser.apellidoPaterno,
      role: UserServiceMock.updateUser.role,
      estado: UserServiceMock.updateUser.estado,
    });
